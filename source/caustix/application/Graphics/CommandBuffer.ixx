...
namespace Caustix {
    ...
    void CommandBuffer::BindPass( RenderPassHandle handle_ ) {
        ...
        //if ( !m_isRecording )
        {
            ...
            if ( m_currentRenderPass && ( m_currentRenderPass->m_type != RenderPassType::Compute ) && ( render_pass != m_currentRenderPass ) ) {
            ...
            if ( render_pass != m_currentRenderPass && ( render_pass->m_type != RenderPassType::Compute ) ) {
                ...
                render_pass_begin.framebuffer = render_pass->m_type == RenderPassType::Swapchain ? m_device->vulkan_swapchain_framebuffers[m_device->vulkan_image_index] : render_pass->vk_frame_buffer;
    ...
    void CommandBuffer::BindDescriptorSet( DescriptorSetHandle* handles, u32 num_lists, u32* offsets, u32 num_offsets ) {
        ...
        vkCmdBindDescriptorSets( vk_command_buffer, m_currentPipeline->vk_bind_point, m_currentPipeline->vk_pipeline_layout, k_first_set,
    ...
    void CommandBuffer::DrawIndexed( TopologyType::Enum topology, u32 index_count, u32 instance_count, u32 first_index, i32 vertex_offset, u32 first_instance ) {
...
// DrawIndirect = 0, VertexInput = 1, VertexShader = 2, FragmentShader = 3, RenderTarget = 4, ComputeShader = 5, Transfer = 6
    ...
    void CommandBuffer::Barrier( const ExecutionBarrier& Barrier ) {
        ...
        if ( Barrier.m_newBarrierExperimental != u32_max ) {
            ...
            for ( u32 i = 0; i < Barrier.m_numImageBarriers; ++i ) {
...

                {
                    ...
                    ResourceState current_state = Barrier.m_sourcePipelineStage == PipelineStage::RenderTarget ? RESOURCE_STATE_RENDER_TARGET : RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
                    ResourceState next_state = Barrier.m_destinationPipelineStage == PipelineStage::RenderTarget ? RESOURCE_STATE_RENDER_TARGET : RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
                    if ( !is_color ) {
                        current_state = Barrier.m_sourcePipelineStage == PipelineStage::RenderTarget ? RESOURCE_STATE_DEPTH_WRITE : RESOURCE_STATE_DEPTH_READ;
                        next_state = Barrier.m_destinationPipelineStage == PipelineStage::RenderTarget ? RESOURCE_STATE_DEPTH_WRITE : RESOURCE_STATE_DEPTH_READ;
                    }
            ...
            source_stage_mask = UtilDeterminePipelineStageFlags( source_access_flags, Barrier.m_sourcePipelineStage == PipelineStage::ComputeShader ? QueueType::Compute : QueueType::Graphics );
            destination_stage_mask = UtilDeterminePipelineStageFlags( destination_access_flags, Barrier.m_destinationPipelineStage == PipelineStage::ComputeShader ? QueueType::Compute : QueueType::Graphics );

            vkCmdPipelineBarrier( vk_command_buffer, source_stage_mask, destination_stage_mask, 0, 0, nullptr, Barrier.m_numMemoryBarriers, buffer_memory_Barriers, Barrier.m_numImageBarriers, image_Barriers );
            return;
        }
        ...
        VkPipelineStageFlags destination_stage_mask = TovkPipelineStage( ( PipelineStage::Enum )Barrier.m_destinationPipelineStage );
    ...
    void CommandBuffer::FillBuffer( BufferHandle buffer, u32 offset, u32 size, u32 data ) {
        ...
        vkCmdFillBuffer( vk_command_buffer, vk_buffer->vk_buffer, VkDeviceSize( offset ), size ? VkDeviceSize( size ) : VkDeviceSize( vk_buffer->m_size ), data);
    }
    ...
    void CommandBufferRing::Initialize( GpuDevice* gpu ) {
        ...
        for ( u32 i = 0; i < k_max_pools; i++ ) {
            ...
            check( vkCreateCommandPool( gpu->vulkan_device, &cmd_pool_info, gpu->vulkan_allocation_callbacks, &m_vulkanCommandPools[ i ] ) );
        }
